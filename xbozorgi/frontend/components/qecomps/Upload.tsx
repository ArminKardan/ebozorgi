...
export default (props: {
  ...
  if (typeof window != "undefined") {
    ...
    global.uploaders[props.id].open = () => {
      ...
      if (!!statuses.find(st => st.percent < 100)) {
        alerter(z.lang.uploadwaiterr)
        return
      }
      if (statuses.length >= 5) {
        alerter(z.lang.uploadmaxerr)
        return
      }
      statuses = []
      props.on?.(statuses)
  ...
  function renameFiles() {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const hash = SerialGenerator(30).toLowerCase()
      let newFileName = file.name.replace(/[^A-Za-z0-9]/g, '') + "-" + hash + '.' + extension;
    ...
    if (fileinput.files && fileinput.files[0]) {
      for (var f of Array.from(fileinput.files)) {
        statuses.push({
          ...
          size: f.size,
          percent: 0,
          url: `https://cdn.ituring.ir/qeupload/${z.enduser.uid}/${f.name.toLowerCase()}`
        })
      }
    }
    if (fileinput.files && fileinput.files[0]) {
      for (var f of Array.from(fileinput.files)) {
        ...
        body.append("uid", z.enduser.uid.toString());
        if (props.max_age_sec && props.max_age_sec > 0) {
          body.append("max_age_sec", props.max_age_sec.toString() || "");
        }
  ...
  if (props.uploadit) {
    props.uploadit.now = (file) => {
      ...
      if (f.size > (props.maxsize || 12 * 1024 * 1024)) {
        alerter(z.lang.uploaderrsize.replace("XX",
          (Math.ceil((props.maxsize || 12 * 1024 * 1024) / (1024))).toLocaleString(z.lang.region) + " kb"))
        return
      }
      uploadToServer()
    }
    ...
    <div style={{ display: "flex", marginTop: 5, marginLeft: props.marginLeft }}>
      ...
      {statuses.map((st, i) => {
        ...
        if (st.percent == 100) {
          ...
          if (extension == ("jpg") || extension == ("jpeg") || extension == ("png") || extension == ("gif") || extension == ("svg")
            ...
            return <div key={key} style={{ position: "relative", lineHeight: 0.7, marginTop: -5 }}>
              <a style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", paddingRight: 5 }}
                ...
                <img id={props.id + "_img"} style={{ borderRadius: 5, width: 40, height: 40 }} src={st.url}
                  onError={async (e) => {
                    e.currentTarget.src = ""
              ...
              <div className={z.qestyles.imgremove} onClick={() => {
                statuses = statuses.filter(it => !(it.name.toLowerCase() == st.name.toLowerCase()));
                localStorage.setItem(localstorage_key, JSON.stringify(statuses))
                props.on?.(statuses)
                props.onclear?.()
              }}>
                <Img style={{ borderRadius: 5, width: 20, height: 20 }} src={global.cdn("/files/close.svg")} />
              </div>
            </div>
          }
          ...
          return <div key={key} style={{ position: "relative", lineHeight: 0.7, marginTop: -5 }}>
            <a style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", paddingRight: 5 }} target="_blank" href={st.url}>
              <Img style={{ borderRadius: 5, width: 40, height: 40 }} src={global.cdn("/files/file.svg")} />
              <br-xx/>
              <h5>{fname[i] + "." + extension}</h5></a>

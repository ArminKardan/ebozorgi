...
export default (props)=>
{
  ...
  return <div style={{width:"100%", marginTop:5,  fontSize:12}}>
   {props.title}<sup style={{fontSize:8}}>{props.sup}</sup>
  <div style={{display:"flex", alignItems:"center", marginTop:3}}>
   {props.lefticon?<><Img style={{cursor:"pointer", width:28, height:28}} src ={props.lefticon} on={()=>{props.onlefticon?props.onlefticon():null}}/>&nbsp;</>:null}
    <input inputMode={props.type=="currency"?"numeric":null} id={props.id?props.id:null}
     key={defaultValue+"key"} defaultValue={defaultValue} readOnly={props.readOnly} 
    value={props.readOnly?defaultValue:undefined}
    type={props.type||'text'} disabled={props.readOnly} 
    className={z.qestyles.txt3} style={{ width:"calc(100% - 15px)",  paddingLeft:10, marginRight:0, direction:"ltr"}}  spellCheck={false}
    placeholder={""} 
    ...
    <div style={{height:30, overflowY:'hidden', width:250, marginRight:10}}>
      <img src={"https://qepal.com/api/user/captcha?uuid="+ props.uuid} style={{height:50, marginTop:-15}} onClick={()=>{props.reload?.()}}/>
    </div>
  </div>  
</div>
}
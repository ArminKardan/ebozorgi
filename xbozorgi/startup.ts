...
import S10, { APIERLoop } from "@/backend/ROOT/Schedule/S10";
...
import XMPP from "./backend/ROOT/XMPP";
...
export const Run = async () => {
    ...
    for (let p of envs) {
        if (os.platform() == "linux" && p.includes("env-serv"))
            continue
        dotenv.config({ path: p });
    }

    if (!process.env.SERVICE_SECRET)
        throw "NO SERVICE_SECRET FOUND!, PLEASE TAKE THE SERVICE SECRET KEY FILE AND PUT IT ON ENV FOLDER OR ADD SECRET_SERVICE AS AN ENVIRONMENT VARIABLE."

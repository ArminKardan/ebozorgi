...
const LogItem = (props) => {
  ...
  let color = "#0088FF"
  let icon = global.cdn("/files/info.png")

  if (props.type == "ok") {
    color = "yellowgreen"
    icon = global.cdn("/files/ok.svg")
  }
  ...
  let direction = "ltr" as any
  if (typeof props.children == "string") {
    if (FAtoENRatio(props.children) > 0.5) {
      direction = "rtl"
    }
  }

  return <div style={{ color, direction }}>
    <Img src={global.cdn(icon)} style={{
      display: "inline-block", verticalAlign: "middle", width: 12,
      marginTop: props.type == "warning" ? -4 : 0
    }} /><sp-4 />
    <span>{">"} <sp-4 /> {props.children} </span>
  </div>
}
...
export default (props) => {
  ...
  return <>
    ...
    <div className={z.qestyles.dialog}
      style={{
        width: "92vw", maxWidth: props.maxWidth || 370, zIndex: props.wz || (props.z ? props.z + 1 : null),
        // height: 150,
        top: "50%",
      ...
      <c-cc style={{ height: "100%", width: "100%", borderRadius: 5, padding: "10px 0 20px 0", color: "yellowgreen" }}>
        ...
        <br-x />
        <br-x />
        <Img className={z.qestyles.hoverfade} src={global.cdn("/files/forbidden.svg")}
        ...
        {items.length > 0 ? <c-x style={{
          ...
          <c-cc style={{
            position: "absolute", bottom: 5, right: z.lang.dir == "ltr" ? 5 : null,
            left: z.lang.dir == "rtl" ? 5 : null
          }}>
            <Img className={z.qestyles.hoverfadexx} src={global.cdn("/files/share2.svg")}
              style={{ width: 30, }} on={() => {

              }} />
            <br-xx />
            <Img id="loggerbtm" className={z.qestyles.hoverfadexx} src={global.cdn("/files/godown.svg")}
              style={{ width: 30, display: "none", marginTop: 5 }} on={() => {
                let el = document.getElementById("logger")
                el.scrollTo(0, el.scrollHeight)
                setTimeout(() => {

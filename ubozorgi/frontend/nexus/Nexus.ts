...
export default async (z: ZType) => {
    ...
    if (z.middleuser?.uid && !global.xmpp) {
        if (!global.resource) {
            global.resource = "webuo" + Math.floor(Math.random() * 10000).toString()
        }
        global.xmppapicb = {}
        ...
        global.nexus = {
            ...
            subscribe: async (channel: string) => {
                await global.xmpp.send(global.xmppxml(
                    'presence',
                    {
                        to: `${channel + "@conference.qepal.com"}/${global.xmpp_app}-${z.middleuser.uid}-${global.xmpp_role}-${global.resource}`,
                        type: 'available'
                    },
                ));
        ...
        if (json.code != 0) {
            ...
            console.error("Middle User:", z.middleuser)
            return
        }
        ...
        global.xmpp.on('online', async (address) => {
            ...
            console.log("[u-nexus] connected:", global.resource)
